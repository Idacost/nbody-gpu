...
int main(int argc, char* argv[]) {
  ...
  for (size_t step = 0; step < nbstep; ++step) {
    ...
    int blockSize = 256;
    int numBlocks = (nbpart + blockSize - 1) / blockSize;
    calculate_forces<<<numBlocks, blockSize>>>(d_mass, d_x, d_y, d_z, d_fx, d_fy, d_fz, nbpart, G);
    cudaDeviceSynchronize();

    update_positions_velocities<<<numBlocks, blockSize>>>(d_mass, d_x, d_y, d_z, d_vx, d_vy, d_vz, d_fx, d_fy, d_fz, nbpart, dt);
    cudaDeviceSynchronize();
}
